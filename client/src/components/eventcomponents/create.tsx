...
const CreateEvent = () => {
  ...
  >([
    { name: 'Sub Event 1', venue: 'Delhi', startTime: newDate, endTime: newDate },
    { name: 'Sub Event 2', venue: 'Delhi', startTime: newDate, endTime: newDate },
    {
      name: 'Reception',
      venue: 'Delhi',
      startTime: newDate,
  ...
  return (
    <div className="flex justify-center items-center px-14 py-20 w-full h-screen font-josefin">
      {!subEventsSelected ? (
            ...
            >
              <div className="flex flex-col gap-4 w-full h-full">
                <div className="flex flex-col gap-1">
                  <h2>Sub events are a great way to divide your whole event into sub parts and organizing each sub event with their respective participants seperately.</h2>
                  <h3>Atleast one sub event is needed to continue forward!</h3>
                </div>
                <div className="flex justify-between">
                  <span>{eventData.name.substring(0, 200) + ".."} sub events: {subEvents.length}</span>
                  <button
                    type="button"
                    onClick={() =>
                      setSubEvents((prev) => [
                    ...
                    className="bg-primary-light ml-auto px-2 py-1 rounded-md w-40 text-white flex items-center gap-1"
                  >
                    <Add /> Add Sub Event
                  </button>
                </div>
                <div className="flex flex-col gap-2 max-h-[320px] w-full overflow-y-scroll">
                  {subEvents.map((subEvent, index) => (
                    <div key={index} className="flex flex-row gap-2">
                      ...
                      />
                      <label className="flex flex-row gap-3 justify-center items-center">
                        <input
                          className="border-primary-light bg-background-light text-[18px] w-[30vw] my-1 px-2 py-1 border-b focus:border-b-2 focus:outline-none"
                          type="text"
                          value={subEvent.name}
                ...
                {subEvents.length > 0 && (
                  <div className="flex flex-row justify-between mt-2">
                    <button
                      ...
                      children={'Cancel'}
                      className="border-black border px-3 py-1 rounded-md"
                    />
                    <button
                      onClick={() => {
        ...
        <div className="flex flex-col items-center justify-center border-2 border-primary-light px-4 py-2 w-2/3 h-[85vh]">
          <StepsPanes
            subEvents={subEvents}
