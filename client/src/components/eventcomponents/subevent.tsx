...
const Subevents = () => {
  ...
  const createSubEvent = async (e, isEdit = false) => {
    try {
      ...
      const res = await callApi(uri, method, {
        ...
        channelId: Number(subEvent?.id || 0),
      })
      console.log(res)
      if (res.status === 201) {
        setSubEvent({
  ...
  const deleteSubEvent = async (subEvent) => {
    ...
    const res = await callApi('/channel', 'DELETE', {
      eventId: Number(eventId),
      channelId: subEvent?.id,
    })
    if (res.status === 204) {
      setSnackbar({
        open: true,
    ...
    <div className="flex-col px-4 md:px-10 w-full font-josefin container">
      <Box className="flex justify-between items-center bg-background-extralight my-4 px-4 md:px-6 py-4">
        <span className="font-bold text-xl">
          Total Sub Events: {channel?.length}
        </span>
        <button
          type="button"
          className="flex items-center gap-1 bg-primary-light hover:bg-transparent ml-auto px-2 py-1 border hover:border-black rounded-md w-40 text-white hover:text-black transition-all"
          onClick={handleOpen}
        >
          <Add /> Add Sub Event
        </button>
      ...
      >
        <Box className="flex flex-col gap-2 bg-white px-4 py-4 rounded-md w-3/4 h-fit">
          <h2 className="font-semibold text-lg">Create Sub Event</h2>
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1 pt-1 pb-4">
              <h2>
                You can add different event participants and groups for each sub
                event.
              </h2>
              <h3>
                Default groups will be created which include groups for
                Announcements, Vendor, General and Photos
              </h3>
            </div>
            <form
            ...
            >
              <input
                ...
                value={subEvent.name}
                onChange={(e) =>
                  setSubEvent((prev) => ({ ...prev, name: e.target.value }))
                }
                required
              />
              <textarea
                ...
                value={subEvent.desc}
                onChange={(e) =>
                  setSubEvent((prev) => ({ ...prev, desc: e.target.value }))
                }
                required
              />
              <input
                ...
                onChange={(e) =>
                  setSubEvent((prev) => ({ ...prev, venue: e.target.value }))
                }
                required
              />
      ...
      >
        <Box className="flex flex-col gap-2 bg-white px-4 py-4 rounded-md w-3/4 h-fit">
          <h2 className="mb-4 font-semibold text-lg">Edit Sub Event</h2>
          <div className="flex flex-col gap-4">
            <form
              className="flex flex-col justify-center items-center gap-4 w-full"
              onSubmit={(e) => createSubEvent(e, true)}
            >
              <input
                placeholder="Sub Event Name Eg. Reception"
                className="border-primary-light bg-background-light my-1 px-2 py-1 border-b focus:border-b-2 w-2/3 text-[18px] focus:outline-none"
      ...
      {channel && channel.length <= 0 ? (
        ...
        channel?.map((item) => (
          <Box
            ...
            display={'flex'}
            gap={4}
            className="flex md:flex-row flex-col justify-between items-center border-2 hover:border-primary-light bg-background-extralight hover:bg-white my-2 px-4 md:px-6 py-2 border-transparent rounded-md transition-all"
          >
