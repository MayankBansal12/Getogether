...
router.post('/setup', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: 'Table setup completed!', table: eventTable })
  } catch (error) {
    res
      .status(500)
      .json({ message: 'Error setting up the table!', error: error.message })
  }
})
...
router.post('/event', async (req, res) => {
  ...
  try {
    ...
    if (!tables) {
      return res
        .status(404)
        .json({ message: 'Event not found or no tables assigned!' })
    }

    res
      .status(200)
      .json({ message: 'Fetched all tables for the event!', tables })
  } catch (error) {
    res
      .status(500)
      .json({
        message: 'Error fetching tables for the event!',
        error: error.message,
      })
  }
})
...
router.post('/user', async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: 'Error fetching tables for the user!',
        error: error.message,
      })
  }
})
...
router.put('/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({
        message: 'Updated the status for table in event',
        bookTable: status,
      })
  } catch (error) {
    res
      .status(500)
      .json({
        message: 'Error chaning the booking status',
        error: error.message,
      })
  }
})
  ...
  .post(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (existingAssignment) {
        return res
          .status(400)
          .json({ message: 'Table is already assigned to a user!' })
      }
      ...
      res
        .status(201)
        .json({ message: 'Table assigned to the user!', table: singleTable })
    } catch (error) {
      console.log(error)
      res
        .status(500)
        .json({ message: 'Error assigning the table!', error: error.message })
    }
  })
  .delete(async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({
          message: 'Error cancelling the table assignment!',
          error: error.message,
        })
    }
  })
